(() => {
    // Solo ejecutar en páginas de LinkedIn Sales Navigator
    ...
    async function extractFirstProfile() {
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    // Primero intentamos obtener la tarjeta de perfil
    const profileCard = document.querySelector('li.artdeco-list__item, tr.search-results__result-item');
    
    if (!profileCard) {
        throw new Error("No se encontró ningún perfil en la página de búsqueda");
    }

    // Para el nombre
    const name = profileCard.querySelector('.artdeco-entity-lockup__title')?.textContent.trim() || 
                profileCard.querySelector('.name-link')?.textContent.trim() || 
                "Desconocido";

    // Para el título y empresa
    const title = profileCard.querySelector('.artdeco-entity-lockup__subtitle')?.textContent.trim() || "";
    const description = profileCard.querySelector('.artdeco-entity-lockup__metadata')?.textContent.trim() || "";

    // Buscar el "Acerca de" con el nuevo método
    let about = "";
    
    // Buscar todo el texto en el perfil
    const allText = profileCard.textContent || document.body.textContent;
    
    // Buscar el texto que comienza con "Acerca de:" usando expresión regular
    const aboutMatch = allText.match(/Acerca de:([\s\S]*?)(?=(?:Mensaje|Guardar|Contactos en común|Publicaciones|$))/i);
    
    if (aboutMatch && aboutMatch[1]) {
        about = aboutMatch[1].trim();
    }

    // Si no encontramos el about con el método anterior, intentar con selectores directos
    if (!about) {
        const aboutSelectors = [
            'dt:-webkit-any(dt:has(+ dd:not(:empty)))',
            'dd.about-preview__body',
        ...
        for (const selector of aboutSelectors) {
            ...
            if (element) {
                ...
                if (text && !text.includes("Tus comentarios nos ayudan")) {
                    about = text;
                    break;
                }
            }
  ...
  async function handleDataExtraction() {
    ...
    // Función principal que maneja la extracción según la página
    async function handleDataExtraction() {
        ...
        try {
            ...
            if (window.location.href.includes("/sales/search")) {
                data = await extractFirstProfile();
            } else {
                ...
                data = await extractConversationData(selectedConversation);
            }
            ...
            button.innerHTML = window.location.href.includes("/sales/search") ? "Extraer Perfil" : "Settear";
            button.disabled = false;
            button.style.opacity = "1";
            button.style.cursor = "pointer";
        }
